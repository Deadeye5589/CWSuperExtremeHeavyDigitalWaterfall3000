...
// Defines ---------------------------------------------------------
#define BUFFER_SIZE				4		//Size of the UART Rx ring buffer
#define DRV8860DELAY			4		//Delay in 탎 for DRV8860 drivers
...
void init_ports(void){
	...
	PORTD.DIRSET = (1<<PORT7) | (1<<PORT2) | (1<<PORT1) | (1<<PORT0);	//Out for DRV8860 Enable, Data Out, Clock and Latch
}
...
{
 	uint8_t Data = USARTC0.DATA;							//Receive 1 byte from UART
	...
	{	
		...
		*/
		...
		USARTC0.DATA = 0x0A;
		/*
		Send content of ring buffer 
		*/	
		PORTD.OUTSET = PIN0_bm;									//Latch High
		PORTD.OUTCLR = PIN1_bm;									//Clock Low
		_delay_us(DRV8860DELAY);								//Delay 4탎
		PORTD.OUTCLR = PIN0_bm;									//Latch High
		for(uint8_t i = 0; i < BUFFER_SIZE; i++)
		{
		DRV8860_send_data(RingBuffer_Load(&Buffer));								//Send data to first DRV8860 driver
		}
		_delay_us(DRV8860DELAY);								//Delay 4탎
		PORTD.OUTSET = PIN0_bm;									//Latch High
		for(uint8_t i = 0; i < BUFFER_SIZE; i++)
		{	
			while(!(USARTC0.STATUS & USART_DREIF_bm));		//Check that byte was send and transmission buffer is empty
			USARTC0.DATA = RingBuffer_Load(&Buffer);		//Load byte from ring buffer into transmission buffer 
		}
		/*
		Send CR + LF
		*/
...
int main(void)
{
	...
	PORTD.OUTSET = PIN7_bm;									//Enable the DRV8860 output stages
	DRV8860_cleanup();										//Initialize DRV8860 drivers 
