...
void DRV8860_cleanup(uint16_t no_of_drivers){
	...
	{
		while(!(USARTD0.STATUS & USART_DREIF_bm));  //Warten, bis Sendedatenregister frei ist
        USARTD0.DATA = 0;			//Send data to first DRV
        while(!(USARTD0.STATUS & USART_TXCIF_bm));  //Warten, bis Daten durchs Schieberegister rausgetaktet sind
        USARTD0.STATUS=USART_TXCIF_bm;        //Sendeflag löschen (bleibt sonst aktiv)			//Wait for SPI transmission buffer to be empty
	}
	_delay_us(DRV8860DELAY);								//Delay 4us
	PORTD.OUTSET = PIN4_bm;									//Latch High
}
...
int main(void)
{
    ...
    while (1) 
    {
		if(byte_ready){
			...
			{
                while(!(USARTD0.STATUS & USART_DREIF_bm));  //Warten, bis Sendedatenregister frei ist
                USARTD0.DATA = RingBuffer_Load(&Buffer);			//Send data to first DRV
                while(!(USARTD0.STATUS & USART_TXCIF_bm));  //Warten, bis Daten durchs Schieberegister rausgetaktet sind
                USARTD0.STATUS=USART_TXCIF_bm;        //Sendeflag löschen (bleibt sonst aktiv)			//Wait for SPI transmission buffer to be empty
			}
			_delay_us(DRV8860DELAY);								//Delay 4us
