...
volatile uint8_t config = 0;			//Status Counter for the config state machine

// Enumerations -------------------------------------------------------------
enum {start, framelenght_1, framelenght_2, payload, setup};		//Status of state machine to be stored in state_count
...
ISR(USARTC0_RXC_vect)
{
	...
	{
		frame_lenght = data;									//The third byte needs to be the low byte of the 16 bit frame length value
	...
	else if (state_count == payload && frame_lenght > 0)		//All other bytes are payload of a frame
	{
		...
		else if (state_count == setup){				//Check if byte "C" indicates setup routine and statemachine is at start
		//to do EEPROM storage of this byte
		state_count = start;
	}
